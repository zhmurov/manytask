...
from .plugins.firejail import disable_firejail
...
def _disable_firejail() -> None:
    """Disable firejail sandboxing for `safe_run_script` stages in this process."""
    disable_firejail()
    print_info(
        "Firejail sandboxing disabled: tested code runs unsandboxed, "
        "with full network and filesystem access of the current user",
        color="orange",
    )
...
@click.option(
    "--no-firejail",
    is_flag=True,
    help="Run tested code without the firejail sandbox (firejail is Linux-only)",
)
@click.option(
    "-v/-s",
    "--verbose/--silent",
...
def grade(  # noqa: PLR0913
    ...
    Firejail is Linux-only; pass --no-firejail to grade on a host without it.
    """
    ...
    if all_tasks:
        ...
        if not tasks_to_grade:
            # An explicit --all-tasks that matches nothing is a mistake, not an empty run.
            # The usual cause is swapped arguments: tasks are looked up in REFERENCE_ROOT.
            print_info("No enabled tasks found in the reference", color="red")
            print_info(
                f"Looked for */{Course.TASK_CONFIG_NAME} in the reference root {reference_root} "
                f"and matched them against enabled groups in {MANYTASK_CONFIG}.\n"
                f"Usage is `checker grade ROOT REFERENCE_ROOT`, where ROOT is the student "
                f"repository and REFERENCE_ROOT is the private one - check they are not swapped.",
            )
            sys.exit(1)
        print_info(
