...
from checker.plugins.firejail import is_firejail_disabled
...
def captured_run(monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    ...
    def fake_run(
        ...
        captured["no_firejail"] = is_firejail_disabled()
...
def _reset_firejail_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    """The switch is a process-global; never leak it between tests."""
...
class TestGradeTaskSelection:
    ...
    def test_all_tasks_fails_when_nothing_matches(
        self, tmp_path: Path, course_root: Path, captured_run: dict[str, Any]
    ) -> None:
        """A reference with the configs but no tasks (e.g. swapped arguments) must fail loudly."""
        empty_reference = tmp_path / "empty"
        empty_reference.mkdir()
        (empty_reference / CHECKER_YML_NAME).write_text(CHECKER_YML)
        (empty_reference / MANYTASK_YML_NAME).write_text(MANYTASK_YML)

        result = CliRunner().invoke(
            cli, ["grade", str(course_root), str(empty_reference), "--all-tasks"]
        )

        assert result.exit_code == 1
        assert "No enabled tasks found in the reference" in result.output
...
class TestGradeNoFirejail:
    def test_flag_disables_firejail_for_the_run(
        self, course_root: Path, captured_run: dict[str, Any]
    ) -> None:
        result = run_grade(course_root, "--all-tasks", "--no-firejail")

        assert result.exit_code == 0, result.output
        assert captured_run["no_firejail"] is True
        assert "Firejail sandboxing disabled" in result.output

    def test_firejail_enabled_by_default(
        self, course_root: Path, captured_run: dict[str, Any]
    ) -> None:
        result = run_grade(course_root, "--all-tasks")

        assert result.exit_code == 0, result.output
        assert captured_run["no_firejail"] is False
        assert "Firejail sandboxing disabled" not in result.output
